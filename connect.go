...
func upsertOpenCodeProvider(configPath, providerName, baseURL, apiKey string, models []string, includeClaude bool) error {
	return upsertOpenCodeProviderOpts(configPath, providerName, baseURL, apiKey, models, includeClaude, "", false)
}

func upsertOpenCodeProviderOpts(configPath, providerName, baseURL, apiKey string, models []string, includeClaude bool, proxyURL string, zdr bool) error {
	...
	if includeClaude {
		if err := upsertClaudeKey(doc, apiKey, proxyURL, zdr); err != nil {
			return err
		}
	}
...
func persistYAML(configPath string, original []byte, root *yaml.Node) error {
	...
	backup := configPath + ".commandcode-session.bak"
...
func applyZDRHeader(target *yaml.Node, zdr bool) {
	...
	if zdr {
		...
		mappingSet(headers, zdrHeader, scalarNode("1"))
		return
	}
	if headers != nil && headers.Kind == yaml.MappingNode {
		mappingDelete(headers, zdrHeader)
		if len(headers.Content) == 0 {
			mappingDelete(target, "headers")
		}
	}
}
...
func lookupConfiguredEntry(entries []configuredEntry, providerName, name string) (configuredEntry, int, bool) {
	...
	if strings.HasPrefix(name, "commandcode-") {
		if idx, err := strconv.Atoi(strings.TrimPrefix(name, "commandcode-")); err == nil && idx >= 1 && idx <= len(entries) {
