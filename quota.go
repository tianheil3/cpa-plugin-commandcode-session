...
func (q *quotaAdapter) FetchQuota(ctx context.Context, req pluginapi.QuotaFetchRequest) (pluginapi.QuotaFetchResponse, error) {
	if !isOpenCodeQuotaRequest(req, q.p.cfg.providerName()) {
		return pluginapi.QuotaFetchResponse{}, fmt.Errorf("credential is not Command Code")
	}
	...
	if apiKey == "" {
		return pluginapi.QuotaFetchResponse{}, fmt.Errorf("no Command Code API key on this credential")
	}
	if matched.BaseURL != "" {
	...
	usage, err := fetchZenUsageAt(ctx, baseURL, apiKey, q.p.cfg.CPAConfigPath)
	if err != nil {
...
func (q *quotaAdapter) ResetQuota(context.Context, pluginapi.QuotaResetRequest) (pluginapi.QuotaResetResponse, error) {
	return pluginapi.QuotaResetResponse{
		Success: false,
		Message: "Command Code quotas reset on the official rolling/weekly/monthly windows; they cannot be cleared locally.",
	}, nil
}
...
func usageToQuota(usage zenUsage) pluginapi.QuotaFetchResponse {
	...
	return pluginapi.QuotaFetchResponse{
		Subscription: &pluginapi.QuotaSubscription{Plan: "Command Code", TierName: planName(usage.Plan)},
...
func isOpenCodeQuotaRequest(req pluginapi.QuotaFetchRequest, providerName string) bool {
	...
	if req.Attributes != nil {
		...
		if strings.Contains(base, "commandcode.ai") {
			return true
		}
	}
