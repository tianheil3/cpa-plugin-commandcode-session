#!/usr/bin/env python3
"""Patch CLIProxyAPI management.html so Command Code appears on /quota and / (auth files).
...
MARKER = "commandcode"
SHARE_MARK = "data-commandcode-auth-share"
TOKEN_MARK = "data-commandcode-token-panel"
SHARE_SCRIPT = """  <script data-commandcode-auth-share="1">
...
FETCH_QUOTA_OLD = (
    ...
    "if(!n)throw Error(t(`commandcode_quota.missing_auth_index`));"
...
LIST_FILES_NEW = (
    ...
    "if(String(s?.name||``).trim().toLowerCase()!==`commandcode`)continue;"
    "let c=s[`api-key-entries`],h=0;"
    "Array.isArray(c)&&c.forEach((u,d)=>{"
    "let f=u?.[`auth-index`]??u?.authIndex??u?.auth_index;"
    "n.push({name:`commandcode-${d+1}`,provider:`openai-compatible-commandcode`,"
    "type:`openai-compatible-commandcode`,auth_index:f?String(f):``,authIndex:f?String(f):``,"
    "label:s.name||`commandcode`,disabled:!1,runtime_only:!0});h++});"
    "h||n.push({name:`commandcode`,provider:`openai-compatible-commandcode`,"
    "type:`openai-compatible-commandcode`,label:s.name||`commandcode`,"
...
AUTH_LIST_NEW = (
    ...
    "try{let t=await op.get(`/plugins/commandcode-session/auth-files`),n=t?.files||[];"
    "if(Array.isArray(n)&&n.length){try{let r=await op.get(`/openai-compatibility`),"
    "a=(r?.[`openai-compatibility`]||[]).find(s=>String(s?.name||``).trim().toLowerCase()===`commandcode`),"
...
CARD_MODELS_NEW = "C=!x||S===`aistudio`||S===`commandcode`"
HEALTH_TEXT_OLD = "te=x?t(`auth_files.type_virtual`)"
HEALTH_TEXT_NEW = "te=x&&S!==`commandcode`?t(`auth_files.type_virtual`)"
HEALTH_CLASS_OLD = "ne=x?uk.stateVirtual"
HEALTH_CLASS_NEW = "ne=x&&S!==`commandcode`?uk.stateVirtual"
QUOTA_ROW_OLD = "j=!!A&&!x&&!r"
QUOTA_ROW_NEW = "j=!!A&&(!x||S===`commandcode`)&&!r"
ND_OLD = "nD=new Set([`antigravity`,`claude`,`codex`,`kimi`,`xai`])"
ND_NEW = "nD=new Set([`antigravity`,`claude`,`codex`,`kimi`,`xai`,`commandcode`])"
RD_OLD = "rD=[`vertex`,`aistudio`,`antigravity`,`xai`,`claude`,`codex`,`kimi`]"
RD_NEW = "rD=[`vertex`,`aistudio`,`antigravity`,`xai`,`claude`,`codex`,`kimi`,`commandcode`]"
GET_MODELS_OLD = (
...
GET_MODELS_NEW = (
    ...
    "try{let r=await op.get(`/plugins/commandcode-session/auth-files`),"
...
CARD_DELETE_NEW = (
    "t(`auth_files.models_button`)]}),S===`commandcode`&&(0,H.jsx)(U,{variant:`danger`,size:`sm`,"
...
TOGGLE_WRAP_NEW = "(!x||S===`commandcode`)&&(0,H.jsxs)(`div`,{className:uk.toggleWrap"
SET_STATUS_OLD = "setStatus:(e,t)=>op.patch(`/auth-files/status`,{name:e,disabled:t})"
SET_STATUS_NEW = (
    "setStatus:async(e,t)=>String(e||``).startsWith(`commandcode-`)||String(e||``).includes(`openai-compatibility:commandcode:`)"
    "?op.patch(`/plugins/commandcode-session/auth-files/status`,{name:e,disabled:t})"
...
DELETE_FILES_NEW = (
    ...
    "let n=t.filter(e=>String(e||``).startsWith(`commandcode-`)||String(e||``).includes(`openai-compatibility:commandcode:`)),"
    "r=t.filter(e=>!(String(e||``).startsWith(`commandcode-`)||String(e||``).includes(`openai-compatibility:commandcode:`))),"
    "i={deleted:0,files:[],failed:[]};"
    "if(n.length){let a=cy(await op.delete(`/plugins/commandcode-session/auth-files`,{data:{names:n}}),n);"
...
RU_FILTER_NEW = '"filter_kimi":"Kimi","filter_commandcode":"Command Code","filter_aistudio":"AIStudio"'
...
STORE_MEMO_NEW = 'w=(0,y.useMemo)(()=>({antigravity:v,claude:b,codex:x,kimi:S,xai:C,["commandcode"]:Og}),[v,b,x,S,C,Og])'
...
FILTER_FN_OLD = (
    ...
    "||t===`openai-compatibility:commandcode`||t.endsWith(`-commandcode`)||t.endsWith(`:commandcode`))&&!c_(e)}"
)
FILTER_FN_NEW = (
    "filterFn:e=>{let t=n_(e);return t===`commandcode`||t===`openai-compatible-commandcode`"
...
I18N = {
    "Kimi 额度": (
        "commandcode_quota:{title:`Command Code 额度`,empty_title:`暂无 Command Code 渠道`,"
        "empty_desc:`在 Command Code 插件页接入 API Key 后即可在此查看额度。`,"
        "idle:`点击此处刷新额度`,loading:`正在加载额度...`,"
        "load_failed:`额度获取失败：{{message}}`,missing_auth_index:`认证文件缺少 auth_index`,"
        "empty_models:`暂无额度数据`}"
    ),
    "Kimi 配額": (
        "commandcode_quota:{title:`Command Code 配額`,empty_title:`暫無 Command Code 渠道`,"
        "empty_desc:`在 Command Code 外掛頁接入 API Key 後即可在此查看配額。`,"
        "idle:`點此重新整理配額`,loading:`正在載入配額...`,"
        "load_failed:`配額取得失敗：{{message}}`,missing_auth_index:`驗證檔案缺少 auth_index`,"
        "empty_models:`暫無配額資料`}"
    ),
    "Kimi Quota": (
        "commandcode_quota:{title:`Command Code Quota`,empty_title:`No Command Code credentials`,"
        "empty_desc:`Connect an Command Code API key on the plugin page to view quota here.`,"
    ...
    "Квота Kimi": (
        "commandcode_quota:{title:`Квота Command Code`,empty_title:`Нет учётных данных Command Code`,"
        "empty_desc:`Подключите ключ Command Code на странице плагина, чтобы увидеть квоту.`,"
...
def replace_if(html: str, old: str, new: str, label: str, count: int | None = 1) -> str:
    ...
    # would otherwise keep appending (e.g. `||S===commandcode`).
    if new in html:
        return html
    if old not in html:
        raise SystemExit(f"missing needle {label}")
...
def upgrade_auth_files_patch(html: str) -> str:
    """Show Command Code on the official auth-files page with models + health."""
    html = collapse_repeated(html, "||S===`commandcode`")
...
def upgrade_quota_patch(html: str) -> str:
    """Keep an already-patched panel matching the current Command Code quota rules."""
...
def inject_compat_files(html: str) -> str:
    """Keep quota + auth-file pages sharing Oy.list(), including Command Code keys."""
...
def finalize(html: str) -> str:
    ...
    if "||S===`commandcode`||S===`commandcode`" in html:
        raise SystemExit("duplicated commandcode models condition")
...
def patch(html: str) -> str:
    ...
    adapter_done = (
        ...
        and "Yj=[`claude`,`antigravity`,`codex`,`xai`,`kimi`,`commandcode`]" in html
    )
    if adapter_done:
    ...
    html = must_replace(
        ...
        "var Yj=[`claude`,`antigravity`,`codex`,`xai`,`kimi`,`commandcode`]",
        "Yj",
    )
    html = must_replace(
        html,
        "Zj={antigravity:qD.filterFn,claude:yO.filterFn,codex:FO.filterFn,kimi:BO.filterFn,xai:JO.filterFn}",
        'Zj={antigravity:qD.filterFn,claude:yO.filterFn,codex:FO.filterFn,kimi:BO.filterFn,xai:JO.filterFn,["commandcode"]:rk["commandcode"].filterFn}',
        "Zj",
    )
    html = must_replace(
        html,
    ...
    html = must_replace(
        ...
        "n===`xai`?e.xaiQuota[t.name]:n===`commandcode`?e.opencodeGoQuota[t.name]:ck(n)",
        "store lookup",
    )
    html = must_replace(
        html,
    ...
    html = must_replace(
        html,
        "filter_kimi:`Kimi`,",
        'filter_kimi:`Kimi`,"filter_commandcode":`Command Code`,',
        "filter labels",
        count=3,
    )
    ...
    for block, payload in [
        ...
        inserted += 1
    if inserted != 4 or html.count("commandcode_quota:{") != 4:
        raise SystemExit("failed to inject commandcode_quota i18n")
    return finalize(inject_compat_files(html))
