...
func TestManagementRegisterABIOmitsHandler(t *testing.T) {
	...
	for _, route := range decoded.Routes {
		if route.Path == "/plugins/commandcode-session/auth-files" && route.Method == http.MethodGet {
			foundAuthFiles = true
			break
		}
	}
...
func TestHandleManagementResourceHTML(t *testing.T) {
	...
	resp, err := p.HandleManagement(context.Background(), pluginapi.ManagementRequest{
		...
		Path:   "/v0/resource/plugins/commandcode-session/status",
	})
	if err != nil {
		t.Fatal(err)
	}
	body := string(resp.Body)
	if !strings.Contains(body, "订阅池") || !strings.Contains(body, "Command Code") {
		t.Fatalf("html = %s", body)
	}
	if !strings.Contains(body, "本机累计 Token") {
		t.Fatal("dashboard must show local token totals")
	}
	if !strings.Contains(body, "/plugins/commandcode-session/tokens/reset") {
		t.Fatal("dashboard must expose token reset")
	}
	if !strings.Contains(body, `window.__COMMANDCODE_STATUS__ = {`) {
...
func TestHandleConnectRequiresAPIKey(t *testing.T) {
	...
	resp, err := p.HandleManagement(context.Background(), pluginapi.ManagementRequest{
		...
		Path:   "/v0/management/plugins/commandcode-session/connect",
		Body:   []byte(`{}`),
	})
	if err != nil {
		t.Fatal(err)
...
func TestHandleConnectWritesProvider(t *testing.T) {
	...
	keys, models, err := listConfiguredKeys(path, "commandcode")
	if err != nil {
		t.Fatal(err)
	}
	...
	keys, _, err = listConfiguredKeys(path, "commandcode")
	if err != nil {
		t.Fatal(err)
	}
	if len(keys) != 2 {
...
func TestHandleAuthFilesListsConfiguredKeys(t *testing.T) {
	...
	p := &sessionPlugin{cfg: defaultConfig()}
	p.cfg.CPAConfigPath = path
	firstID := stableCompatAuthID("commandcode", "sk-aaaaaaaaaaaaaaaaaaaa", "https://api.commandcode.ai/provider/v1", "direct")
	...
	resp, err := p.HandleManagement(context.Background(), pluginapi.ManagementRequest{
		Method: http.MethodGet,
		Path:   "/v0/management/plugins/commandcode-session/auth-files",
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	wantFirst := opencodeAuthFileName(configuredEntry{
		APIKey: "sk-aaaaaaaaaaaaaaaaaaaa", BaseURL: "https://api.commandcode.ai/provider/v1", ProxyURL: "direct",
	}, "commandcode")
	wantSecond := opencodeAuthFileName(configuredEntry{
		APIKey: "sk-bbbbbbbbbbbbbbbbbbbb", BaseURL: "https://api.commandcode.ai/provider/v1", ProxyURL: "direct",
	}, "commandcode")
	if first.Name != wantFirst || second.Name != wantSecond {
		t.Fatalf("names = %q %q want %q %q", first.Name, second.Name, wantFirst, wantSecond)
	}
	if first.Type != "commandcode" || first.Provider != "commandcode" || !first.RuntimeOnly || first.Disabled {
...
func TestHandleAuthFilesStatusAndDelete(t *testing.T) {
	...
	resp, err := p.HandleManagement(context.Background(), pluginapi.ManagementRequest{
		...
		Path:   "/v0/management/plugins/commandcode-session/auth-files/status",
	...
	entries, _, err := listConfiguredEntries(path, "commandcode")
	if err != nil {
		t.Fatal(err)
	}
	...
	second := opencodeAuthFileName(entries[0], "commandcode")
	resp, err = p.HandleManagement(context.Background(), pluginapi.ManagementRequest{
		Method: http.MethodDelete,
		Path:   "/v0/management/plugins/commandcode-session/auth-files",
	...
	raw, _ = os.ReadFile(path)
	if strings.Contains(string(raw), "name: commandcode") {
		t.Fatalf("commandcode channel still present:\n%s", raw)
	}
	if !strings.Contains(string(raw), "name: other") {
...
func TestIsOpenCodeQuotaRequest(t *testing.T) {
	...
	ok := isOpenCodeQuotaRequest(pluginapi.QuotaFetchRequest{
		...
		Attributes: map[string]string{"compat_name": "commandcode"},
	}, "commandcode")
	if !ok {
		t.Fatal("expected commandcode compat credential")
	}
