...
func TestQuotaProviderAliasesIncludeCompatRuntimeKey(t *testing.T) {
	t.Parallel()
	got := quotaProviderAliases("commandcode")
	want := []string{
		"commandcode",
		"openai-compatibility:commandcode",
		"openai-compatible-commandcode",
	}
...
func TestIsOpenCodeQuotaRequestMatchesRuntimeProvider(t *testing.T) {
	...
	if !isOpenCodeQuotaRequest(req, "commandcode") {
		t.Fatal("expected runtime openai-compatible-commandcode credential to match")
	}
	...
	if isOpenCodeQuotaRequest(other, "commandcode") {
...
func TestUsageToQuotaUsesRemainingFraction(t *testing.T) {
	...
	if got.Subscription == nil || got.Subscription.Plan != "Command Code" {
