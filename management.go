...
func (p *sessionPlugin) RegisterManagement(_ context.Context, _ pluginapi.ManagementRegistrationRequest) (pluginapi.ManagementRegistrationResponse, error) {
	return pluginapi.ManagementRegistrationResponse{
		Routes: []pluginapi.ManagementRoute{
			{Method: http.MethodGet, Path: "/plugins/commandcode-session/status", Handler: p},
			{Method: http.MethodGet, Path: "/plugins/commandcode-session/auth-files", Handler: p},
			{Method: http.MethodPatch, Path: "/plugins/commandcode-session/auth-files/status", Handler: p},
			{Method: http.MethodDelete, Path: "/plugins/commandcode-session/auth-files", Handler: p},
			{Method: http.MethodPost, Path: "/plugins/commandcode-session/connect", Handler: p},
		...
		Resources: []pluginapi.ResourceRoute{
			{
				Path:        "/status",
				Menu:        "Command Code",
				Description: "Connect multiple Command Code subscriptions; requests pick the key with the most remaining quota.",
				Handler:     p,
			},
		},
	}, nil
...
func (p *sessionPlugin) HandleManagement(ctx context.Context, req pluginapi.ManagementRequest) (pluginapi.ManagementResponse, error) {
	...
	case req.Method == http.MethodGet && strings.HasSuffix(path, "/plugins/commandcode-session/status"):
		return p.handleStatus(ctx)
	case req.Method == http.MethodGet && strings.HasSuffix(path, "/plugins/commandcode-session/auth-files"):
		return p.handleAuthFiles(ctx)
	case (req.Method == http.MethodPatch || req.Method == http.MethodPost) && strings.HasSuffix(path, "/plugins/commandcode-session/auth-files/status"):
		return p.handleAuthFilesStatus(ctx, req.Body)
	case req.Method == http.MethodDelete && strings.HasSuffix(path, "/plugins/commandcode-session/auth-files"):
		return p.handleAuthFilesDelete(ctx, req)
	case req.Method == http.MethodPost && strings.HasSuffix(path, "/plugins/commandcode-session/connect"):
		return p.handleConnect(ctx, req.Body)
	case req.Method == http.MethodPost && strings.HasSuffix(path, "/plugins/commandcode-session/sync-models"):
		return p.handleSyncModels(ctx, req.Body)
	case req.Method == http.MethodPost && strings.HasSuffix(path, "/plugins/commandcode-session/refresh"):
		return p.handleStatus(ctx)
...
func (p *sessionPlugin) dashboardPage(ctx context.Context) string {
	...
	return strings.Replace(dashboardHTML, "window.__COMMANDCODE_STATUS__ = null;", "window.__COMMANDCODE_STATUS__ = "+string(raw)+";", 1)
}
...
func (p *sessionPlugin) handleConnect(ctx context.Context, body []byte) (pluginapi.ManagementResponse, error) {
	...
	usage, errUsage := fetchZenUsageAt(ctx, p.cfg.zenBaseURL(), apiKey, p.cfg.CPAConfigPath)
	if errUsage != nil {
...
func (p *sessionPlugin) handleSyncModels(ctx context.Context, body []byte) (pluginapi.ManagementResponse, error) {
	...
	if apiKey == "" {
		return jsonResponse(http.StatusBadRequest, map[string]any{"error": "no Command Code API key configured"}), nil
	}
	models, err := fetchZenModelsAt(ctx, p.cfg.zenBaseURL(), apiKey, p.cfg.CPAConfigPath)
	if err != nil {
		return jsonResponse(http.StatusBadGateway, map[string]any{"error": err.Error()}), nil
	}
	if err := upsertOpenCodeProviderOpts(path, p.cfg.providerName(), p.cfg.zenBaseURL(), apiKey, models, false, p.cfg.ProxyURL, p.cfg.ZDR); err != nil {
		return jsonResponse(http.StatusInternalServerError, map[string]any{"error": err.Error()}), nil
	}
...
func (p *sessionPlugin) statusPayload(ctx context.Context) (map[string]any, error) {
	...
	for _, entry := range entries {
		...
		usage, errUsage := fetchZenUsageAt(ctx, base, entry.APIKey, p.cfg.CPAConfigPath)
		if errUsage != nil {
...
func htmlResponse(body string) pluginapi.ManagementResponse {
	return pluginapi.ManagementResponse{
		...
		Headers: http.Header{
			...
			"Pragma":                    {"no-cache"},
			"X-CommandCode-Session-Plugin": {pluginVersion},
		},
		Body: []byte(body),
	}
}
