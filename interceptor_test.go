...
func TestInterceptDoesNotInjectSessionHeader(t *testing.T) {
	t.Parallel()
	...
	resp, err := p.InterceptRequestAfterAuth(context.Background(), pluginapi.RequestInterceptRequest{
		...
		Metadata: map[string]any{
			"session_affinity_provider": "openai-compatible-commandcode",
		},
	})
	if err != nil {
		t.Fatal(err)
	}
	if got := resp.Headers.Get(zdrHeader); got != "" {
		t.Fatalf("zdr default off, got %q", got)
	}
	if got := resp.Headers.Get("x-opencode-session"); got != "" {
		t.Fatalf("must not inject x-opencode-session, got %q", got)
	}
}

func TestInterceptSetsZDRWhenEnabled(t *testing.T) {
	t.Parallel()
	cfg := defaultConfig()
	cfg.ZDR = true
	...
	if got := resp.Headers.Get(zdrHeader); got != "1" {
...
func TestInterceptSkipsNonOpenCodeModels(t *testing.T) {
	...
	if got := resp.Headers.Get(zdrHeader); got != "" {
		t.Fatalf("non-Command Code request got x-cmd-zdr = %q", got)
	}
	if len(resp.Body) != 0 {
	...
	if got := after.Headers.Get(zdrHeader); got != "" {
		t.Fatalf("codex request got x-cmd-zdr = %q", got)
	}
}

func TestShouldInjectSessionUsesProviderAndAuthID(t *testing.T) {
	t.Parallel()
	authIDs := map[string]struct{}{"openai-compatibility:commandcode:abc123abc123": {}}
	models := []string{"minimax-m3"}
	if shouldInjectSession(pluginapi.RequestInterceptRequest{
		Model: "deepseek-v4.1-flash",
	}, "commandcode", models, authIDs) {
		t.Fatal("unrelated model must not inject before auth")
	}
	if !shouldInjectSession(pluginapi.RequestInterceptRequest{
		Model:    "minimax-m3",
		ToFormat: "openai",
		Metadata: map[string]any{"session_affinity_provider": "openai-compatible-commandcode"},
	}, "commandcode", models, authIDs) {
		t.Fatal("Command Code provider must inject")
	}
	if shouldInjectSession(pluginapi.RequestInterceptRequest{
		Model:    "minimax-m3",
		ToFormat: "openai",
