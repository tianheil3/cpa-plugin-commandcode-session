...
extern int CommandCodeSessionPluginCall(char*, uint8_t*, size_t, cliproxy_buffer*);
extern void CommandCodeSessionPluginFree(void*, size_t);
extern void CommandCodeSessionPluginShutdown(void);
*/
import "C"
...
func cliproxy_plugin_init(host *C.cliproxy_host_api, plugin *C.cliproxy_plugin_api) C.int {
	...
	plugin.call = C.cliproxy_plugin_call_fn(C.CommandCodeSessionPluginCall)
	plugin.free_buffer = C.cliproxy_plugin_free_fn(C.CommandCodeSessionPluginFree)
	plugin.shutdown = C.cliproxy_plugin_shutdown_fn(C.CommandCodeSessionPluginShutdown)
	return 0
}

//export CommandCodeSessionPluginCall
func CommandCodeSessionPluginCall(method *C.char, request *C.uint8_t, requestLen C.size_t, response *C.cliproxy_buffer) C.int {
	if response != nil {
		response.ptr = nil
		response.len = 0
	}
...
//export CommandCodeSessionPluginFree
func CommandCodeSessionPluginFree(ptr unsafe.Pointer, _ C.size_t) {
	if ptr != nil {
		C.free(ptr)
	}
}

//export CommandCodeSessionPluginShutdown
func CommandCodeSessionPluginShutdown() {
	abiState.Lock()
...
func handleRegister(request []byte) ([]byte, error) {
	...
	if !ok || p == nil {
		return nil, fmt.Errorf("commandcode-session registration returned invalid interceptor")
	}
	abiState.Lock()
	abiState.plugin = p
	abiState.shuttingDown = false
...
func beginPluginCall() (*sessionPlugin, func(), error) {
	...
	if abiState.shuttingDown {
		return nil, nil, fmt.Errorf("commandcode-session plugin is shutting down")
	}
	if abiState.plugin == nil {
		return nil, nil, fmt.Errorf("commandcode-session plugin is not registered")
	}
